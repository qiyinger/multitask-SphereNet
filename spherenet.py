...
class ResidualLayer(torch.nn.Module):
    def __init__(self, hidden_channels, act=activation_resolver('swish')):
...
class init(torch.nn.Module):
    def __init__(self, num_radial, hidden_channels, act=activation_resolver('swish'), use_node_features=True):
...
class update_e(torch.nn.Module):
    def __init__(self, hidden_channels, int_emb_size, basis_emb_size_dist, basis_emb_size_angle, basis_emb_size_torsion,
                 ...
                 num_before_skip, num_after_skip, act=activation_resolver('swish')):
...
class SphereNet(torch.nn.Module):
    ...
    def __init__(
            ...
            act=activation_resolver('swish'), output_init='GlorotOrthogonal', use_node_features=True):
...
class MultiSphereNet(torch.nn.Module):
    def __init__(
            self, energy_and_force=False, cutoff=5.0, num_layers=4,
            hidden_channels=128, out_channels=1, int_emb_size=64,
            basis_emb_size_dist=8, basis_emb_size_angle=8, basis_emb_size_torsion=8, out_emb_channels=256,
            num_spherical=7, num_radial=6, envelope_exponent=5,
            num_before_skip=1, num_after_skip=2, num_output_layers=3):
        super(MultiSphereNet, self).__init__()

        self.sphereNet = SphereNet(energy_and_force=energy_and_force,
                                   cutoff=cutoff, num_layers=num_layers,
        ...
        self.homo_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                         nn.ReLU(),
                                         nn.Linear(out_channels, 1))

        self.lumo_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                         nn.ReLU(),
                                         nn.Linear(out_channels, 1))

        self.gap_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                        nn.ReLU(),
                                        nn.Linear(out_channels, 1))
        self.r2_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                      nn.ReLU(),
                                      nn.Linear(out_channels, 1))

        self.zpve_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                         nn.ReLU(),
                                         nn.Linear(out_channels, 1))

        self.U0_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                         nn.ReLU(),
                                         nn.Linear(out_channels, 1))

        self.U_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                         nn.ReLU(),
                                         nn.Linear(out_channels, 1))

        self.H_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                         nn.ReLU(),
                                         nn.Linear(out_channels, 1))

        self.G_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                      nn.ReLU(),
                                      nn.Linear(out_channels, 1))

        self.Cv_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                      nn.ReLU(),
                                      nn.Linear(out_channels, 1))

        self.mu_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                       nn.ReLU(),
                                       nn.Linear(out_channels, 1))

        self.alpha_linear = nn.Sequential(nn.Linear(out_channels, out_channels),
                                       nn.ReLU(),
                                       nn.Linear(out_channels, 1))

    def forward(self, batch_data, target):
        feat = self.sphereNet(batch_data)
        if target == 'task1':
            return feat

        elif target == 'task2':
            return self.zpve_linear(feat), self.U0_linear(feat), \
                   self.U_linear(feat), self.H_linear(feat), \
                   self.G_linear(feat), self.Cv_linear(feat)

        else:
            return self.mu_linear(feat), self.alpha_linear(feat)
